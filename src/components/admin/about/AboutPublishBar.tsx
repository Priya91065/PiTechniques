...
export default function AboutPublishBar({ canPublish, canEdit }: { canPublish: boolean; canEdit: boolean }): JSX.Element {
  ...
  const loadStatus = useCallback(async (): Promise<void> => {
    try {
      const res = await fetch("/api/admin/about-page/status");
  ...
  async function publish(): Promise<void> {
    const ok = await confirm({
      ...
      message: "Your draft changes to the Banner, Intro and Agile process sections will go live on the About page.",
      confirmText: "Publish",
    });
    if (!ok) return;
    setBusy(true);
    try {
      const res = await fetch("/api/admin/about-page/publish", { method: "POST" });
      const data: { error?: string } = await res.json().catch(() => ({}));
      if (!res.ok) {
        notify(data.error ?? "Publish failed", "error");
        return;
      }
      notify("About page published");
      setHasChanges(false);
    } catch {
  ...
  async function discard(): Promise<void> {
    const ok = await confirm({
      ...
      message: "The Banner, Intro and Agile process sections will be reverted to the last published version.",
      confirmText: "Discard",
      destructive: true,
    });
    if (!ok) return;
    setBusy(true);
    try {
      const res = await fetch("/api/admin/about-page/discard", { method: "POST" });
      const data: { error?: string } = await res.json().catch(() => ({}));
      if (!res.ok) {
    ...
    <Paper sx={{ p: 2, mb: 3 }}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems={{ sm: "center" }} justifyContent="space-between">
        ...
        <Stack direction="row" spacing={1}>
          <Button size="small" variant="text" component="a" href="/about" target="_blank" rel="noreferrer" endIcon={<OpenInNewIcon />}>
            Preview live
          </Button>
          {canEdit && (
      ...
      <Box sx={{ mt: 1.5 }}>
        <Alert severity="info" variant="outlined" sx={{ py: 0.5 }}>
          Edits to the <strong>Banner</strong>, <strong>Intro</strong> and <strong>Agile process</strong> sections are saved as a draft and only appear on the live About page after you publish.
        </Alert>
      </Box>
    </Paper>
  );
