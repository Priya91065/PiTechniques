(() => {
  let img;
  let dots = [];
  let detailLevel = 12; // Adjust for performance
  ...
  function setup() {
    ...
    pixelDensity(1);
    frameRate(30); // Limit FPS

    img = loadImage(
      BASE_URL + 'images/banner-img/about-animation-banner.avif',
      () => {
        img.resize(200, 0); // Resize before processing
        imgLoaded = true;
        buildDots(); // ✅ central function
  ...
  function windowResized() {
    ...
    if (imgLoaded) {
      buildDots(); // ✅ rebuild on resize/orientation change
    }
  }
