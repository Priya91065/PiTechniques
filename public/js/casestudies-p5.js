...
const IMG_PATH =
  ...
  (isMobileView
    ? 'images/banner-img/casestudy-mobile.jpg'   // 👈 mobile image
    : 'images/banner-img/case-studies-banner.jpg');        // 👈 desktop image
  const DETAIL_LEVEL = 6;    // pixel step (lower = denser)
  const MIN_DOT_RADIUS = 0.1;
  const MAX_DOT_RADIUS = 4;
  const DOT_COLOR = '#F4F4F4';
  const HOVER_COLOR = '#F7941E';
  const DOT_OPACITY = 1;      // transparency (0–1)
  const FRAME_RATE = 40;
  const BRIGHTNESS_THRESHOLD = 10; // skip dark/black pixels (0–255)
  // -----------------------
  ...
  function resizeCanvas(w) {
    ...
    canvas.style.setProperty('height', canvasH + 'px', 'important'); // !important
...
function buildDots() {
  ...
  // draw cropped area of image into sample canvas
  sampleCtx.drawImage(
    img,
    offsetX, offsetY, drawWidth, drawHeight, // source crop
    0, 0, sampleW, sampleH                   // destination fit
  );
  ...
  // --- Touch & Mouse Events with scroll-friendly logic ---
  let touchStartY = 0;
  let touchStartX = 0;
  ...
  function onTouchMove(e) {
    ...
    // Only block scroll if mostly horizontal or tap
  ...
  // --- Init Event Listeners (iOS Safari compatible) ---
  resizeCanvas(container.offsetWidth);
